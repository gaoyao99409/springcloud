...
public class DmCal2 {
    ...
    private static boolean search(GraphMatch graphMatch , Integer orderIndex){
        ...
        for(int dmIndex = 0 ; dmIndex < graphMatch.getEdges()[0].length ; dmIndex ++){
            //没有连线
            if(graphMatch.getEdges()[orderIndex][dmIndex] <= 0 ){
                continue;
            }
            //已经检测过该节点
            if(graphMatch.getCheckedPath()[dmIndex]){
                continue;
            }
            //设置该节点已经检测过
            graphMatch.getCheckedPath()[dmIndex] = true;
            //递归查找 当前Y节点是否是未覆盖点 或 是否在增广路径上（让已匹配节点去试一下连其他节点）
            if(graphMatch.getPath()[dmIndex] == -1 || search(graphMatch , graphMatch.getPath()[dmIndex])){
                //设置找到路径
                graphMatch.getPath()[dmIndex] = orderIndex;
                return true;
            }
        }
        return false;
