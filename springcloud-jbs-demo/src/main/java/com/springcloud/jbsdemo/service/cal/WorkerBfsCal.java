...
public class WorkerBfsCal {
        ...
        for (ScriptWorkerBO scriptWorkerBO : scriptWorkerRole.getScriptWorkerList()) {
            ...
            try {
                ...
                if (conflictOrderList.size() > 0 && findOrdersRoleWorker(conflictOrderList, scriptWorkerBO.getWorkerId())) {
                    ...
                    //恢复之前的选择状态
                    workerUsedMap = bak;
                    restoreOrderByUsedMap(this.jbsOrderBOList, workerUsedMap);
                }
            } finally {
     ...
     * @param map
     */
    public static void restoreOrderByUsedMap(List<JbsOrderBO> orderList, Map<Long, List<JbsOrderBO>> map) {
        for (JbsOrderBO order : orderList) {
            for (ScriptWorkerRoleBO roleBO : order.getScript().getScriptWorkerRoleList()) {
                for (ScriptWorkerBO workerBO : roleBO.getScriptWorkerList()) {
                    workerBO.setSelected(false);
                    if (map.containsKey(workerBO.getWorkerId()) && map.get(workerBO.getWorkerId()).contains(order)) {
                        workerBO.setSelected(true);
                    }
                }
            }
        }
